...
export class TuotetiedotUrl extends HTMLElement{
    constructor(){
        super();
    ...
    attributeChangedCallback(attribuutti,vanha,uusi){
        fetch(uusi,{mode:'cors'}).then(data=>data.json()).then(tulos=>{
            if(tulos.length>0){
...
if(!customElements.get('tuote-url')){
    customElements.define('tuote-url',TuotetiedotUrl);
}